...
class NMAC(BasicMAC):
    ...
    def forward(self, ep_batch, t, test_mode=False):
        ...
        if self.central_mac is not None:
            central_agent_outs, self.central_hidden_states = self.central_mac.agent(agent_inputs, self.central_hidden_states)
            return agent_outs.view(ep_batch.batch_size, self.n_agents, -1), agent_outs_2.view(ep_batch.batch_size, self.n_agents, -1),agent_outs_3.view(ep_batch.batch_size, self.n_agents, -1), central_agent_outs.view(ep_batch.batch_size, self.n_agents, -1)
    ...
    def init_hidden(self, batch_size):
        ...
        if self.central_mac is not None:
            self.central_hidden_states = self.central_mac.agent.init_hidden().unsqueeze(0).expand(batch_size, self.n_agents, -1)  # bav
