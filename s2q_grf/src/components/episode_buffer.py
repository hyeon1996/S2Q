...
class ReplayBuffer(EpisodeBatch):
    ...
    def insert_episode_batch(self, ep_batch, min_return=None):
        is_updated = True

        if min_return is not None:
            data = ep_batch.data.transition_data
            eps = data['reward'].reshape(-1, self.max_seq_length).sum(-1) >= min_return
            eps = eps.nonzero().reshape(-1)
            n_high_return_eps = eps.shape[0]

            ep_batch.batch_size = n_high_return_eps

            for k, v in data.items():
                data[k] = v[eps]

            if n_high_return_eps == 0:
                is_updated = False
